...
router.post("/", async (request, response, next) => {
	...
	let result;
	try {
		result = await UserModel.create(request.body);
	} catch (error) {
		error.status = 400;
...
// 	let foundUser = await UserModel.findOne({username: request.body.username}).exec().select("+password").exec();;
...
router.post("/jwt", async (request, response, next) => {
	...
	try {
		let foundUser = await UserModel.findOne({ username: request.body.username }).select("+password").exec();

		if (!foundUser) {
			return next(new Error("No user found with that username."));
		}
